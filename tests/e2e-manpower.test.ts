...
import { fetchManpowerConfirm, fetchWeeklySummary } from '../adapters/atlas/commands/_manhours.js';
import {
  flattenManpowerTree,
...
describe('E2E: 实际工时 (Actual) 数据流', () => {
  ...
  it('实际工时透视：按周展开', () => {
    ...
    expect(pivot.rows[0]?.weekHours['2025-06/c1']).toBeCloseTo(10, 4);
  });
  ...
  it('实际工时按月/角色/部门汇总', () => {
    ...
    expect(monthSummary[0]!.total).toBeCloseTo(18, 4);

    const roleSummary = summarizeActual(rows, 'role', {});
    expect(roleSummary.find((e) => e.key === 'role:产品')?.total).toBeCloseTo(10, 4);
    expect(roleSummary.find((e) => e.key === 'role:研发')?.total).toBeCloseTo(8, 4);

    const deptSummary = summarizeActual(rows, 'department', {});
    expect(deptSummary.find((e) => e.key === 'dept:065527')?.total).toBeCloseTo(18, 4);
  });
...
describe('E2E: 基线 ↔ 实际数据交叉校验', () => {
  ...
  it('2025-04 实际(22) ≤ 基线(25)', async () => {
    ...
    expect(actualTotal).toBeCloseTo(22, 4);
...
describe('E2E: 特殊边界情况', () => {
  it('string 类型 manpower 被正确解析', () => {
    ...
    expect(pivotActualRows(rows, {}).rows[0]?.total).toBeCloseTo(7.5, 4);
  });
  ...
  it('null/zero/negative manpower 被跳过', () => {
    ...
    expect(pivotActualRows(rows, {}).rows[0]?.total).toBeCloseTo(3, 4);
  });
