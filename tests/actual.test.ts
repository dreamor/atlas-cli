...
describe('pivotActualRows', () => {
  ...
  it('computes total from monthly actuals', () => {
    const pivot = pivotActualRows(rows, {});
    expect(pivot.rows[0]?.total).toBe(15);
    expect(pivot.rows[1]?.total).toBe(10);
  });
  ...
  it('respects --from filter', () => {
    ...
    expect(pivot.rows[1]?.total).toBe(3);
  });

  it('respects --to filter', () => {
    const pivot = pivotActualRows(rows, { to: '2025-04' });
    // Only 2025-04 entries should remain
    expect(pivot.rows[0]?.total).toBe(10); // 5+5
    expect(pivot.rows[1]?.total).toBe(7);  // 3+4
  });
  ...
  it('handles string-valued manpower', () => {
    ...
    expect(pivot.rows[0]?.total).toBe(7.5);
  });
  ...
  it('skips null/zero/negative manpower', () => {
    ...
    expect(pivot.rows[0]?.total).toBe(3);
  });
});
...
describe('summarizeActual', () => {
  ...
  it('summarizes by month', () => {
    ...
    expect(apr?.total).toBeCloseTo(22.5, 1); // (5+3+2.5) + (5+4+3) in Apr
    expect(may?.total).toBeCloseTo(10.5, 1); // 5+3+2.5 in May
  });
  ...
  it('summarizes by role', () => {
    ...
    expect(product?.total).toBe(15);
    expect(dev?.total).toBeCloseTo(18, 1); // 10+8
  });
  ...
  it('summarizes by department (team lead)', () => {
    ...
    expect(fan?.total).toBeCloseTo(25, 1); // 15+10
    expect(zhao?.total).toBeCloseTo(8, 1);
  });
  ...
  it('respects --from filter in summary', () => {
    ...
    expect(entries[0]?.total).toBeCloseTo(10.5, 1);
  });
...
describe('renderActualSummaryTable', () => {
  ...
  it('renders month summary with total row', () => {
    ...
    expect(out).toContain('actual hours');
...
describe('full pipeline: flatten → filter → pivot', () => {
  it('end-to-end with sampleTree, no filter', () => {
    ...
    expect(pivot.rows[0]?.total).toBe(15);
    expect(pivot.rows[1]?.total).toBe(10);
    expect(pivot.rows[2]?.total).toBeCloseTo(8, 1);
  });
  ...
  it('pipeline with nested tree', () => {
    ...
    const pivot = pivotActualRows(rows, {});
    expect(pivot.rows.reduce((sum, r) => sum + r.total, 0)).toBeCloseTo(12, 0); // 5+4+3
  });
});
